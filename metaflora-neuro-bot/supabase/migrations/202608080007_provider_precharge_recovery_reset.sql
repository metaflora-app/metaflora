...
-- A pre-charge selector failure never created an external payment.  It is safe
-- to restart only this verified state after the bounded pre-charge attempts
-- were exhausted; ambiguous or externally-started charges remain untouched.
...
BEGIN
  ...
  SET status = 'queued',
      ...
      updated_at = now()
  WHERE topup.id = p_id
    AND topup.status IN ('manual', 'queued')
