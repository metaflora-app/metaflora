...
-- The live balance belongs to the first fulfillment attempt. A YooKassa retry
-- may arrive after the user has already spent the credited metacoins, so the
-- idempotency lookup must happen before validating p_balance_after.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    v_duplicate := true;
    RETURN QUERY SELECT v_ledger_id, v_duplicate, v_existing.balance_after;
    RETURN;
  END IF;

  IF p_balance_after < p_metacoins THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'metacoin purchase balance is below the credited amount';
  END IF;
...
BEGIN
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT
      ...
      v_existing.starts_at, v_existing.expires_at;
    RETURN;
  END IF;

  IF p_balance_after < p_metacoins THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'subscription balance is below the credited amount';
  END IF;
