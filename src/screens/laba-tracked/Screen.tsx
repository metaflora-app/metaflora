/**
 * экран отслеживаемые лаба (7:1138)
 * Laba Tracked Accounts Screen - 1180x2550px
 */


import { ScreenRoot } from '../../components/layout';

export default function LabaTrackedScreen() {
  return (
    <ScreenRoot className="bg-[#020101] relative overflow-hidden">
      ...
      <div className="relative w-[1180px] h-[2550px] mx-auto">
        ...
        <div className="absolute top-[549px] left-[125px] w-[930px] space-y-[25px]">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-full h-[249px] bg-white/10 backdrop-blur rounded-[30px] border border-white/30 p-[30px] flex items-center gap-[20px]">
              <div className="w-[120px] h-[120px] rounded-full bg-gradient-to-br from-purple-900/20 to-blue-900/20" />
              <div className="flex-1">
                <h3 className="text-white text-[32px] font-['Inter'] font-bold">@username</h3>
                <p className="text-white text-[25px] font-['Gotham_Pro'] font-light">250к подписчиков</p>
              </div>
              <button className="w-[150px] h-[50px] bg-black backdrop-blur rounded-[62px] border border-white/30">
                <span className="text-white text-[20px] font-['Gotham_Pro'] font-medium">отписаться</span>
              </button>
            </div>
          ))}
        </div>
